...
export default function CompleteProfile() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (saveError) {
      ...
      // Continue anyway and save to localStorage as fallback
    } else {
      console.log('Profile saved to backend successfully!');
    }

    // Also save to localStorage for offline access and quick loading
    const userData = {
      username: formData.username,
    ...
    localStorage.setItem('profitAnalysisUser', JSON.stringify(userData));
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {ageError && (
                <div className="flex items-start space-x-2 p-4 bg-red-50 border-2 border-red-200 rounded-lg">
                  ...
                  <p className="text-sm text-red-700">{ageError}</p>
                </div>
              )}
