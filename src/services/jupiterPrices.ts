// CoinGecko API with API Key
const COINGECKO_API_KEY = 'CG-umqwfFMbypZob1NezJCSvoHj';
  ...
  try {
    ...
    Object.entries(TOKEN_IDS).forEach(([symbol, coinGeckoId]) => {
      ...
      if (priceData && priceData.usd) {
        ...
        console.log(`${symbol}: $${price}`); // Debug log
      }
    });
    ...
    ['BTC', 'ETH', 'SOL', 'USDC', 'USDT'].forEach((symbol) => {
      if (!prices.has(symbol)) {
        ...
        if (cached) {
          ...
          console.log(`${symbol}: $${cached.price} (cached)`);
        } else {
          ...
          console.log(`${symbol}: $${defaultPrice} (fallback)`);
        }
      }
    });
 ...
 * @param usdAmount - Amount in USD (plan hardcap like 500, 2000)
